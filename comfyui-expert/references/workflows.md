...
> All templates are subordinate to `state/inventory.json`, `foundation/hardware-profile.md`,
> and current ComfyUI/node version constraints. Node class names and filenames shown here are
> patterns, not proof that a dependency is installed.
...
[Load Checkpoint]
       ↓
[Load LoRA] ← "example_character.safetensors"
       ↓
[PuLID Model Loader]
       ↓
...
**Load LoRA**
```
lora_name: "example_character.safetensors"
strength_model: 0.8
strength_clip: 0.8
```
...
```
"example_character, photorealistic portrait, [character appearance],
 natural lighting, 8k"
```
...
## Workflow 4: Wan 2.2 Image-to-Video
...
```
[Load Diffusion Model: inventory-verified Wan 2.2 I2V/TI2V]
         ↓
...
```
model: "<exact Wan 2.2 filename from state/inventory.json>"
```
...
### VRAM Optimization (for Wan 2.2 14B)
...
```
quantize: "fp8"
attention: "<installed supported attention backend>"
```
