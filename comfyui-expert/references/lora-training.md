...
> All recommendations are subordinate to `state/inventory.json`,
> `foundation/hardware-profile.md`, and current trainer/model version constraints.
...
```
example_character, person, looking at viewer, white shirt, indoor, soft lighting
```

**Critical captioning rules:**

1. **Always prefix with unique trigger word:**
   - Good: `example_character`, `ohwx_person`, `sks_subject`
   - Bad: `woman`, `redhead`, `character` (too generic)

2. **Don't describe the face in detail:**
   - Bad: "person with detailed identity features but no trigger token"
   - Good: "example_character, person, indoor portrait"
...
train_data_dir: "./dataset/example_character"
output_dir: "./output"
output_name: "example_character_sdxl"
...
config:
  ...
  process:
    - type: sd_trainer
      ...
      datasets:
        - folder_path: "./dataset/example_character"
          caption_ext: "txt"
          default_caption: "example_character"
          resolution: [1024]
      ...
      sample:
        sample_every: 250
        prompts:
          - "example_character, photorealistic portrait"
```
...
mkdir -p dataset/example_character/10_example_character
...
- `10_example_character` = each image repeated 10 times per epoch
...
accelerate launch train_network.py --config_file ./config/example_character_train.yaml
```
...
# Include style tags
captions: "example_character, anime style, ..."
```
