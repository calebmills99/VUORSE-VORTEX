...
if __name__ == "__main__":
    clips = [
        "{COMFYUI_PATH}/output/clip_001.mp4",
        "{COMFYUI_PATH}/output/clip_002.mp4",
        "{COMFYUI_PATH}/output/clip_003.mp4"
    ]

    concatenator = VideoConcatenator(output_dir="{COMFYUI_PATH}/output/final")

    if concatenator.validate_clips(clips):
        # Method 1: Fast, no transitions
        # concatenator.concatenate_simple(clips, "character_video_final.mp4")

        # Method 2: With transitions
        concatenator.concatenate_with_transitions(
            clips,
            fade_duration=0.5,
            output_name="character_video_final.mp4"
        )
```
