...
> **Historical changelog.** Model lists and performance notes below describe their dated release
> context. Current inventory, hardware, model-landscape, and version constraints take precedence.
...
**GPU / OS / ComfyUI version:** see `foundation/hardware-profile.md` and
`state/inventory.json`. Do not restate them here -- they drift.

**Capabilities:**
- Can run Wan 2.2 14B natively
- LTX-2 4K generation without optimization
- Batch multiple workflows simultaneously
- Use the precision supported by the target GPU; this machine uses FP8/GGUF, not NVFP4
...
### Project: Example Multi-Shot Character Video

**Goal:** Multi-shot narrative video
**Character:** Character A (`[character appearance]`)
**Style:** Photorealistic, natural lighting, cinematic and character-driven

**Workflow Preferences:**
- Keyframe and identity path selected from inventory
- I2V model selected from verified installed candidates
- Resolution and FPS inherited from the selected workflow and delivery target
- Transitions delegated to `video-assembly`

**What Worked:**
- ✓ Generating and validating keyframes before I2V
- ✓ Using consistent motion prompts with "consistent identity, same person" prefix
- ✓ Controlling queue depth from observed runtime capacity
- ✓ Validating each clip immediately after generation

**What Didn't Work:**
- ✗ Submitting more I2V jobs than the measured queue could carry
