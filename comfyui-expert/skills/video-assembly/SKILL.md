...
Requires an inventory-verified `ffmpeg` executable on PATH.
...
Uses `remotion-best-practices` when that external skill is available.
...
These are repository delivery defaults, not platform requirements. Override them with
the active project's delivery target.
