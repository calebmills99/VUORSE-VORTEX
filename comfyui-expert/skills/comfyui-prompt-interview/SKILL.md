...
description: Clarify vague or underspecified creative intent before prompt construction. Interview for subject, mood, style, and production constraints, then hand a structured brief to the prompt-engineer skill. Use when the user has an idea but has not yet supplied enough detail for a direct prompt. Do not use for direct prompt writing, prompt refinement, workflow building, model training, or troubleshooting.
---
...
[Hand the completed brief to `comfyui-prompt-engineer` when available; otherwise apply the confirmed model-family cues below]
...
- If using an inventory-verified identity method: follow that implementation's facial-description guidance
...
| Identity method | [inventory-confirmed method or "missing dependency"] | [If applicable] |
...
Name only inventory-confirmed stages. If a required identity, detailer, or upscaler
dependency is absent, report it instead of composing a hypothetical stack.
...
|-------------|-----------|
| "Photorealistic, single character, reference images available" | Complete installed reference-identity stack |
| "Multiple characters, consistent identity" | Complete installed multi-identity stack |
| "Fast iteration, character editing" | Installed context-editing model and loader |
...
| "Video, short clip" | Inventory-verified Wan 2.2 TI2V 5B, LTX, or another installed engine |
| "Video, extended sequence" | Inventory-verified long-context or video-extension workflow |
| "Talking head" | Route to video and voice skills; use only complete installed stacks |
