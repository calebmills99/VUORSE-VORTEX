...
Persist observed project and inventory state. Template examples never override the
active manifest, `state/inventory.json`, or `foundation/hardware-profile.md`.
...
hardware:
  gpu: "{inventory.system.gpu}"
  vram_gb: "{inventory.system.vram_total_gb}"
...
```yaml
name: "{character-name}"
trigger_word: "{unique_trigger}"  # e.g., "char_token"
created: "{ISO date}"
