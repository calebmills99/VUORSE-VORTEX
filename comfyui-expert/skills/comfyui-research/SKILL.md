...
description: Research current ComfyUI models, techniques, nodes, and upstream changes using verified sources. Update timestamped reference material and flag stale claims. Use when the user asks for current ecosystem research or when a task depends on guidance already identified as stale.
---
...
- **On demand**: The user asks to research or check for updates.
- **Stale dependency**: The active task depends on a claim flagged by `references/staleness-report.md`.
- **Session start**: Read the staleness report only; do not launch external research automatically.

## Research Pipeline

### 1. Discovery

Search for new developments only after research has been triggered. Prefer official
release notes, repositories, and model cards; use community material as secondary
technique evidence.
...
| Official Wan repositories linked by the Wan-AI publisher | Releases and implementation changes |
...
**From YouTube videos:**
If available, use a transcript-analysis skill to:
1. Get transcript
...
2. If `last_run` exceeds the report's freshness threshold:
   - State the age and identify the specific research task that would refresh it.
3. If any entries are beyond their threshold:
   - Notify: "Stale items found: {list}. These may have newer alternatives."

## Full Research Run

When the user explicitly requests a full ComfyUI research run:
...
4. **Extract findings** with an available transcript-analysis capability; otherwise inspect primary source material directly
